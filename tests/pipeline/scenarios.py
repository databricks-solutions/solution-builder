"""Pipeline compatibility view over canonical scenario contracts.

The durable scenario contract lives in ``evaluation/cases/*.yaml``. This
module keeps the pipeline runner's small dataclass interface while ensuring
all pipeline entry points use the same prompts and expectations.
"""
