...
Country distribution (inside region, ISO-2 codes — used by the world map in `04-ai-bi.md`):
...
**customers**: `customer_id` (PK, CUST-NNNNNN), `email`, `first_name`, `last_name`, `region`, `country` (ISO-2, distributed per region above), `registration_date`, `loyalty_tier`, `premium_status` (`'premium'` / `'not_premium'` / `NULL` per the Premium tagging rule above — drives the model label)
