...
Do NOT create the dashboard until all three checks pass. A dashboard created against missing or empty tables silently fails on every widget with `TABLE_OR_VIEW_NOT_FOUND`, and recovery requires delete-and-recreate.

1. **Pipeline ran successfully.** `resources.json.created_resources.pipeline_id` is set and the latest pipeline update is in a successful terminal state (not `FAILED`, `RUNNING`, or never-started).
2. **Every Gold table referenced by this dashboard exists AND has rows.** Run via `execute_sql` against the target warehouse:
   ```sql
   SELECT COUNT(*) AS n FROM {CATALOG}.{SCHEMA}.gold_daily_summary;
   SELECT COUNT(*) AS n FROM {CATALOG}.{SCHEMA}.gold_daily_orders;
   SELECT COUNT(*) AS n FROM {CATALOG}.{SCHEMA}.gold_returns_by_lot;
   ```
   Each must return `n > 0`. Any `TABLE_OR_VIEW_NOT_FOUND` means the pipeline did not materialize the table — fix `03-pipelines.md` or re-run the pipeline before continuing.
3. **Column names match this spec.** Run `DESCRIBE {CATALOG}.{SCHEMA}.gold_daily_summary` and confirm `date`, `region`, `category`, `total_revenue`, `total_returns_usd` all exist. If any are missing, align the pipeline spec and dashboard spec before proceeding.

Log each validation result inline (e.g., `gold_daily_summary: 1,247 rows ✓`). If any check fails, STOP, fix the pipeline, then retry.
...
Embed the Genie Space (from 04-genie-space.md) into the dashboard. Use the Genie Space ID from `created_resources` in `resources.json`. This lets the persona ask "Why do I have so many returns?" directly from the dashboard.

---

## Resource Tracking

After creating the dashboard, **add the dashboard ID to `created_resources` in `resources.json`**:
```json
{
  "created_resources": {
