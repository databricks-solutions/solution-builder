...
**Row 2 — 4 × `counter` (KPIs with sparklines)**. Source: `ds_daily`. All four use a `period` encoding (weekly `DATE_TRUNC` bucket from the same dataset) so a sparkline renders behind the headline.

- **What the eye sees**: four headline numbers across the top. Refunds + Refund Rate sparklines show a clear bump-then-decay shape; Orders sparkline stays flat — *"the business is fine, but refunds spiked."*
- **Refunds — last 90d** — `SUM(returns_usd)`. Format: currency, compact. Sparkline: spike-then-decay — the visual hook.
...
**Row 4 — `symbol-map` · "Affected customers — bubble map"**. Source: `ds_returns` with widget-level filter `is_bad_lot = TRUE` (apply in the widget's `fields` / WHERE, not on the dataset — keeps the dataset shared with other widgets).
...
- **`pie` (donut) · "Refunds by category"**. Source: `ds_daily`. Slices = `category`, value = `SUM(returns_usd)`.
...
- **`bar` horizontal · "Worst production lots"**. Source: `ds_returns`. y = `lot_id`, x = `COUNT(return_id)`. Sort x DESC. *Metric is count, not rate — the simple demo doesn't carry units_sold per lot.*
...
- **`bar` grouped · "Refunds by country: affected lot vs everyday"**. Source: `ds_returns`. x = `country`, y = `SUM(refund_amount_usd)`, color = `source` (two-value categorical: `Affected lot` → `visualizationColors[3]` = `#1E3A8A` navy, `Everyday returns` → `visualizationColors[4]` = `#7C3AED` violet — palette is 0-indexed in the JSON).
...
- **`bar` horizontal grouped · "Return reasons: affected lot vs everyday"**. Source: `ds_returns`. y = `return_reason` (enum from 01-lakeflow: `quality` / `didnt_fit` / `wrong_item` / `changed_mind`), x = `COUNT(return_id)`, color = `source`.
...
- **`table` · "Recent customer comments"**. Source: `ds_returns`. Columns: `return_date`, `country`, `product_name`, `lot_id`, `return_reason_text` (wider column, wrap). Filter to non-null comments. Sort `return_date` DESC, limit by widget.
  - **What the eye sees**: a scrolling stream of recent return quotes — texture complaints (*"grainy"*, *"separated"*, *"watery"*) cluster on the affected lot. The raw voice of the customer closes the investigation arc with verbatim evidence.
