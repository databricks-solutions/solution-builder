...
function MiniPipeline() {
  return (
    <div className="flex w-full flex-col overflow-hidden rounded-md border border-border/60 bg-background/70" style={{ animation: "gc-in .3s ease-out both" }}>
      ...
      <div className="flex flex-1 items-center justify-center gap-1 px-1.5 py-2">
        {MEDALLION.map((m, i) => (
...
/** The finished dashboard artifact — KPI row, bar chart, and a donut. */
function MiniDashboard() {
  const AIBI = DATABRICKS_ICONS.aibiBrand;
  const KPIS = [
    { c: "#094074", v: "$1.2M", up: true },
    { c: "#3C6997", v: "8.4%", up: true },
    { c: "#FE9000", v: "312", up: false },
  ];
  const BARS = [42, 70, 55, 88, 64, 80, 73];
  return (
    <div className="flex w-full flex-col overflow-hidden rounded-md border border-border/60 bg-background/70" style={{ animation: "gc-in .35s ease-out both" }}>
      <div className="flex items-center gap-1 border-b border-border/60 px-1.5 py-1">
        <AIBI className="h-3 w-3 shrink-0" />
        <span className="truncate font-mono text-[8.5px] text-foreground">dashboard</span>
        <span className="ml-auto rounded bg-emerald-500/15 px-1 text-[7px] font-bold uppercase text-emerald-600">live</span>
      </div>
      <div className="flex flex-1 flex-col gap-1 p-1.5">
        {/* KPI row */}
        <div className="flex gap-1">
          {KPIS.map((k, i) => (
            <span key={i} className="flex flex-1 flex-col rounded-sm px-1 py-0.5" style={{ background: `${k.c}14`, animation: "gc-in .3s ease-out both", animationDelay: `${i * 0.08}s` }}>
              <span className="flex items-center justify-between">
                <span className="text-[7.5px] font-bold leading-none" style={{ color: k.c }}>{k.v}</span>
                <span className="text-[7px] leading-none" style={{ color: k.up ? "#16a34a" : "#ef4444" }}>{k.up ? "▲" : "▼"}</span>
              </span>
              <span className="mt-0.5 h-0.5 w-full rounded-full" style={{ background: `${k.c}44` }} />
            </span>
          ))}
        </div>
        {/* chart row: bars + donut */}
        <div className="flex flex-1 gap-1">
          <div className="flex flex-[2] items-end justify-center gap-[3px] rounded-sm bg-muted/40 px-1 py-0.5" style={{ animation: "gc-in .3s ease-out both", animationDelay: ".12s" }}>
            {BARS.map((h, i) => (
              <span key={i} className="w-[3px] shrink-0 rounded-t-sm" style={{ height: `${h}%`, background: i % 2 ? "#3C6997" : "#FE9000" }} />
            ))}
          </div>
          <div className="grid flex-1 place-items-center rounded-sm bg-muted/40" style={{ animation: "gc-in .3s ease-out both", animationDelay: ".2s" }}>
            <span
