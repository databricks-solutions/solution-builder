...
class ClientErrorReport(BaseModel):
    """Frontend error payload posted to /api/client_errors.

    The reporter (ui/lib/error-reporter.ts) batches errors caught by
    window.onerror, unhandledrejection, and the fetch wrapper.
    """
    error_type: str = Field(..., description="JS error class or 'unhandledrejection' / 'fetch_error'")
    message: str
    stack: Optional[str] = None
    url: Optional[str] = Field(None, description="window.location.href when error fired")
    user_agent: Optional[str] = None
    project_id: Optional[str] = None
    request_id: Optional[str] = Field(None, description="X-Request-Id of the failed API call, if any")
    extra: Optional[dict] = Field(None, description="Free-form extra context")
