...
in-process SDK tools for the Claude Agent SDK.
...
def load_databricks_tools():
    ...
    if _cached_server is not None and _all_tool_names is not None:
        logger.debug(f'Using cached Databricks MCP server with {len(_all_tool_names)} tools')
        return _cached_server, _all_tool_names
    ...
    if not sdk_tools:
        logger.warning("No Databricks MCP tools loaded")
        return None, []
...
# Cached SDK tools and server (loaded once, reused across calls)
_all_sdk_tools = None
...
def _get_all_sdk_tools():
    ...
    try:
        # Import triggers @mcp.tool registration from the local ai_dev_kit packages
        ...
        logger.error(f"Failed to import databricks_mcp_server: {e}")
        return [], []
    ...
    if not registered_tools:
        logger.warning("No tools found in databricks_mcp_server")
        return [], []
...
background execution. When a tool takes longer than 10 seconds, it returns
...
def _create_list_operations_tool():
    ...
    from claude_agent_sdk import tool
    from .operation_tracker import list_operations
...
def _make_wrapper(name: str, description: str, schema: dict, fn):
    ...
    """
    from claude_agent_sdk import tool
    from .operation_tracker import create_operation, complete_operation
    import sys
    import traceback
    ...
    async def wrapper(args: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            cf_future = executor.submit(run_in_context)
            ...
            return {
                'content': [
                    {
                        'type': 'text',
                        'text': f'Error ({type(e).__name__}): {str(e)}\n\nThis error occurred after {elapsed:.2f}s.'
                    }
                ],
                'is_error': True
            }
