...
export function CountryPanel({
  ...
  useEffect(() => {
    // One cancellation flag for both the initial load and every
    // dataMutated-triggered refetch. Without the flag on the subscriber,
    // a stale response landing after a filter change can overwrite the
    // fresh data the new filter just loaded.
    let cancelled = false;
    function reload() {
      ...
      <ul className="divide-y divide-border">
        {rows.map((r) => (
          <CountryRow
            ...
            row={r}
            maxTotal={maxTotal}
            active={selectedCountry === r.country}
            onSelect={() =>
              onCountrySelect(selectedCountry === r.country ? null : r.country)
            }
          />
        ))}
      </ul>
...
function CountryRow({
  ...
  // Pulse the bar when the country's total moves between refetches —
  // e.g. France goes from "20 pending" to "20 approved" after the agent
  // runs, so the pending-scope panel shows it dropping to 0 (or vice-versa
  // when the user flips to the Approved tab).
    ...
    <li>
      ...
      >
        <div className="flex items-center gap-2 sm:gap-3 text-sm">
          ...
          <div
            className="flex-1 h-2.5 rounded-full bg-muted overflow-hidden flex transition-[max-width] duration-500"
